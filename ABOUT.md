...
- **No `styled()`, no `@emotion` import.** MUI through deep default imports only. This is the property that lets one component run under base's emotion engine and league's styled-components engine unchanged, and it is invisible until it breaks.
