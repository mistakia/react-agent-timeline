...
- **Collapsed is one line, and it streams.** The row is derived from `entries`, so it advances on its own as they arrive; it is clamped to a single line because the newest entry is unbounded text and a wrapping row makes the host panel jump on every tick. The line is the newest event itself, with no caption introducing it — the consumer's own heading above the panel is what names the row.
